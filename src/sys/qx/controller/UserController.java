...
public class UserController {
	...
	@RequestMapping("/getOneUserObj")
	public String getOneUserObj(ModelMap model,HttpServletRequest request ,RedirectAttributes redirectAttributes) {
		//获取密码
		...
		//去数据库中查询 看是否有此用户
		UserBean resultUserBean = userDaoMapper.getUser(userName);
		if(resultUserBean != null) {
			//如果有  给与登录  返回  index.html
			if(resultUserBean.getMypwd().equals(password)) {
				System.out.println("------------------------"+resultUserBean.getUserName());
				 request.setAttribute("userOneName",resultUserBean.getUserName()); 
				/* model.addAttribute("myName",123); */
				return "redirect:/index.jsp";
			}else {
				return "redirect:/login_v2.jsp";
			}
		}
		//如果没有  还返回 login.html页面
		return "redirect:/login_v2.jsp";
}
