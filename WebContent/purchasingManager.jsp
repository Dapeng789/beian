...
<body class="gray-bg">
                                ...
                                <table id="exampleTableToolbar" data-mobile-responsive="true" data-show-columns="true">
                                    <thead>
                                        <tr>
											...
											<th data-field="purchasingManagerCode">采购经理审批状态</th>
											<th data-field="operationManagerCode">业务经理审批状态</th>
											<th data-field="reexamineCode">复核状态</th>
											<th data-field="purchasingManagerID">经理外键</th>
											<th data-field="operationManagerID">业务经理外键</th>
											<th data-field="reexamineID">复核员外键</th>
											<th data-field="id">ID编码</th>
                                        </tr>
                                    </thead>
                                    ...
                                    <c:forEach items="${list}" var="usercg" varStatus="st">
										...
										<td>${usercg.code}</td>
										<td>${usercg.purchasingManagerCode}</td>
										<td>${usercg.operationManagerCode}</td>
										<td>${usercg.reexamineCode}</td>
										<td>${usercg.purchasingManagerID}</td>
										<td>${usercg.operationManagerID}</td>
										<td>${usercg.reexamineID}</td>
										<td>${usercg.id}</td>
                                    	</tr>
                                    </c:forEach>
                                </table>
    ...
    function  search(){
		 parent.layer.open({
			    ...
			    shade: 0.8,
			    area: ['1400px', '760px'],
			    content: 'purchasingManagerParticular.jsp'
			     /* end:function(){
