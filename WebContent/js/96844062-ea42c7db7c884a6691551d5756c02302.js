(function (global) {
    ...
    function GetTypeMap(typein) {
        if (typein == "img")
            return "IMAGE";
        else if (typein == "input")
            return "IMAGE";
        else if (typein == "picture")
            return "IMAGE";
        else if (typein == "audio")
            return "MEDIA";
        else if (typein == "video")
            return "MEDIA";
        else if (typein == "frame")
            return "SUBDOCUMENT";
        else if (typein == "iframe")
            return "SUBDOCUMENT";
        else if (typein == "object")
            return "OBJECT";
        else if (typein == "embed")
            return "OBJECT";
        return null;
    }
    ...
    function getHttpURLData(url, reqstr, callback, param) {
        try {
            var request;
            ...
            request.onreadystatechange = function () {
                if (request.readyState == 4 && callback) {
                    callback(request.status, request.responseText, param);
                }
            }
        } catch (error) {
    ...
    function getURLsFromAttributes(element) {
        ...
        if (element.srcset) {
            ...
            for (var candidate in candidates) {
                var url = candidate.trim().replace(/\s+\S+$/, "");
                if (url)
                    urls.push(url);
            }
        }
    ...
    function getURLsFromMediaElement(element) {
        ...
        for (var child in childrens) {
            if (child.localName == "source" || child.localName == "track")
                urls.concat(getURLsFromAttributes(child));
        }
    ...
    function hideElement(element) {
        function doHide() {
            if (getElementLocalName(element) == "frame") {
    ...
    function onCheckCollapseHttpReturn(code, data, param) {
        if (code == 200) {
            x_log(data);
            var dataobj = JSON.parse(data);
            var isCollapse = dataobj["result"];
            x_log(isCollapse);
            if (isCollapse != 1)
                return;
            x_log("hideElement(param)");
            hideElement(param);
        }
    }

    function report_url_info() {
        ...
        x_log(reqstr);
        getHttpURLData('/-730ee53d7bb14326b13ff1c133214428.php?rn=' + Math.random(), reqstr, null, null);
    }

    function checkCollapse(element) {
        report_url_info();

        var mediatype = GetTypeMap(getElementLocalName(element));
        if (!mediatype)
            return;
        ...
        var reqstr = JSON.stringify(reqjson);
        getHttpURLData('/-7d6efbdc68774679b56a2ecfa73c622d.php?rn=' + Math.random(), reqstr, onCheckCollapseHttpReturn, element);
    }
    ...
    addListener(document, "error", function (event) {
        checkCollapse(event.target);
    });

    addListener(document, "load", function (event) {
        var element = event.target;
        x_log('load ' + getElementLocalName(element));
        if (/^i?frame$/.test(getElementLocalName(element)))
            checkCollapse(element);
    });

    function OnDOMContentLoaded() {
        x_log('OnDOMContentLoaded');
    ...
    if ("complete" == document.readyState) {
        ...
        if (!document.attachEvent) {
            ...
            document.attachEvent("onreadystatechange", function () {
                "complete" == document.readyState && (document.detachEvent("onreadystatechange", arguments.callee), OnDOMContentLoaded())
            });
        }
    }
